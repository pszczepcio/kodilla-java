...
public class RemoveProbableNumber extends Sections{
    ...
    public RemoveProbableNumber(SudokuBoard sudokuBoard) {
        this.sudokuBoard = sudokuBoard;
    }

    public void removeNumbersInRow(int row, int column){
        for(int i = 0 ; i < 9 ; i++){
            if(this.sudokuBoard.getvalueOfSingleField(row, i) != -1) {
                filledFieldsList.add(this.sudokuBoard.getvalueOfSingleField(row, i));
            }
        }
        for (int i = 0 ; i < 9 ; i++){
            for(int j = 0 ; j < filledFieldsList.size() ; j++){
                this.sudokuBoard.getProbableNumbers(row, i).remove(filledFieldsList.get(j));
            }
        }
        filledFieldsList.clear();
    }

    public void removeNumbersInColumn(int row, int column){
        for(int i = 0 ; i < 9 ; i++){
            if(this.sudokuBoard.getvalueOfSingleField(i, column) != -1){
                filledFieldsList.add(this.sudokuBoard.getvalueOfSingleField(i, column));
            }
        }
        for (int i = 0 ; i < 9 ; i++){
            for (int j = 0 ; j < filledFieldsList.size() ; j++){
                this.sudokuBoard.getProbableNumbers(i, column).remove(filledFieldsList.get(j));
            }
        }
        filledFieldsList.clear();
    ...
    public void removeNumbersInSection(int blockNumber) {
        switch (blockNumber) {
            case SECTION1: {
                for (int i = 0; i < 3; i++) {
                    for (int j = 0; j < 3; j++) {
                        if (this.sudokuBoard.getvalueOfSingleField(i, j) != -1) {
                            filledFieldsList.add(this.sudokuBoard.getvalueOfSingleField(i, j));
                        }
                    }
                }
                for (int i = 0; i < 3; i++) {
                    for (int j = 0; j < 3; j++) {
                        if (this.sudokuBoard.getvalueOfSingleField(i, j) == -1) {
                            for (int x = 0; x < filledFieldsList.size(); x++) {
                                this.sudokuBoard.getProbableNumbers(i,j).remove(filledFieldsList.get(x));
                            }
                        }
                    }
                }
